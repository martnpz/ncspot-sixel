...
impl SpircTask {
    ...
    fn handle_player_event(&mut self, event: PlayerEvent) -> Result<(), Error> {
        if let PlayerEvent::TrackChanged { audio_item } = event {
            ...
            self.connect_state.set_current_track_uri(audio_item.uri.clone());
        ...
        match event {
            ...
            | PlayerEvent::PositionCorrection { position_ms, .. } => {
                ...
                match self.play_status {
                    ...
                    // Externally-loaded track (player.load() called directly, bypassing Spirc).
                    // Activate the device and transition to Playing so notify() fires and
                    // Spotify's API sees the playing state (Last.fm, activity feeds, etc.).
    ...
    fn handle_next(&mut self, track_uri: Option<String>) -> Result<(), Error> {
        ...
        if has_next_track {
            ...
            // When ncspot manages the queue directly (empty Spirc context), calling handle_stop()
            // would race with ncspot's own queue advancement. Update state only.
            if self.connect_state.context_uri().is_empty() {
                self.play_status = SpircPlayStatus::Stopped {};
                self.connect_state.update_position(0, self.now_ms());
                self.update_state = true;
            } else {
