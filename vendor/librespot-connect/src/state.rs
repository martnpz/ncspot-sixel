...
impl ConnectState {
    ...
    /// Set the current track URI directly, for when the player is loaded externally
    /// (i.e. player.load() called without going through Spirc commands).
    pub fn set_current_track_uri(&mut self, uri: String) {
        use protobuf::MessageField;
        self.player_mut().track = MessageField::some(ProvidedTrack {
            uri,
